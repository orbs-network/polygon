...
class Terraform {
    ...
    createTerraformVariablesFile({ cloud, keys }) {
        ...
        rows.push(
            ...
            { boyarAutoUpdate: cloud.boyarAutoUpdate },
    ...
    async status({ cloud, keys }) {
        ...
        if (fs.existsSync(path.join(this.targetPath, 'terraform.tfvars'))) {
            ...
            if (!currentState.values.outputs) {
                this.spinner.stop();
                return {};
            }
    ...
    async checkManagerLivenessWithAWSCLI({ ip, dns, region }) {
        let ec2 = new AWS.EC2({ apiVersion: '2016-11-15', region });

        try {
            const result = await ec2.describeInstances({}).promise();

            const managerIndex = result.Reservations[0].Instances.findIndex(o => o.PublicDnsName === dns);
            ...
            let managerInstance = result.Reservations[0].Instances[managerIndex];
