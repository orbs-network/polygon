...
class Terraform {
    ...
    async status({ cloud, }) {
        this.spinner = ora('Inspecting your Orbs node').start();

        const { name } = cloud;
        this.setTargetPath(name);
        const { outputs } = await this.terraformPlan({ name });        
        this.spinner.succeed();
    ...
    terraformPlan({ name }) {
        const tfPlanSpawn = spawn('terraform', ['plan', '-var-file=terraform.tfvars', '-refresh'], {
            cwd: this.targetPath,
        });

        let planResults = {
            resourcesToAdd: undefined,
            resourcesToDestroy: undefined,
        };
        ...
        tfPlanSpawn.stdout.on('data', (data) => {
            ...
            if (dataAsString.indexOf('to add,') !== -1) {
                let addRegex = /[0-9]+ to add\,/g;
                let results = dataAsString.match(addRegex);

                let addResults = results[0].match(/[0-9]+/g);
                planResults.resourcesToAdd = parseInt(addResults[0]);

                let destroyRegex = /[0-9]+ to destroy\./g;
                results = dataAsString.match(destroyRegex);
                let destroyResults = results[0].match(/[0-9]+/g);
                planResults.resourcesToDestroy = parseInt(destroyResults[0]);
            }
        });

        return this.executeTerraform(name, 'tf-plan', tfPlanSpawn, () => planResults);
    }
