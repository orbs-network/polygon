/**
 * Main facade for the public Polygon API
 */
...
class Polygon {
    ...
    status({ cloud, keys }) {
        return this.terraform.status({ cloud, keys });
    }

    async createNode({ cloud = {}, keys = {}, }) {
        ...
        if (cloud.name.length > 55) {
            throw new Error('Node name is limited to 55 characters');
        }
        ...
        await this.storeSuccessSnapshot({ result: polygonResult, workspacePath });
    ...
    async destroyNode({ cloud, keys }) {
        if (!cloud.name) {
            console.log('Could not destroy node with the supplied name!', cloud.name);
            return result;
        }
        ...
        await this.storeDestroySnapshot({ result: polygonResult, workspacePath });
