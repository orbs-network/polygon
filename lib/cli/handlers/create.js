...
function _create({ fileMode = false, __fileDirname = '', awsProfile, sshPublicKey: _sshPublicKey,
    ...
    managementConfig,
    boyarVersion = boyarDefaultVersion, _boyarUrl = '', _boyarCommit = '', boyarAutoUpdate = false,
}) {
    let errorMessage,
        boyarTargetUrl = '';
    const baseBoyarURL = `https://github.com/orbs-network/boyarin/releases/download/${boyarVersion}/boyar-${boyarVersion}.bin`;
...
Got: ${orbsPrivateKey} (Length: ${orbsPrivateKey.length})`;
    ...
    if (!_verifyKeys({ orbsAddress, orbsPrivateKey })) {
        errorMessage = `Orbs private key and node address do not match`;
        logRed(errorMessage);
        throw new Error(errorMessage);
    }
    ...
    const params = prepareArgsForPolygonOps({
        ...
        ephemeralStorage, boyarTargetUrl, boyarAutoUpdate,
    });
...
function calcNodeAddressFromPublicKey(publicKey) {
    return Buffer.from(keccak256.arrayBuffer(publicKey).slice(12)).toString('hex');
}

function _verifyKeys({ orbsAddress, orbsPrivateKey }) {
    const ec = new elliptic.ec('secp256k1');
    const keyPair = ec.keyFromPrivate(orbsPrivateKey);
